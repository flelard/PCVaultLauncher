...
export class ConfigPage extends React.Component<ConfigPageProps, ConfigPageState> {
        ...
        return (
            <div className="config-page simple-scroll">
                <div className="config-page__content">
                    ...
                    {/* FlatImage */}
                    <section className="cfg-section">
                        <h2 className="cfg-section__header">FlatImage</h2>
                        <div className="cfg-row">
                            <div className="cfg-row__label">
                                <span className="cfg-row__name">Enable FlatImage Support</span>
                                <span className="cfg-row__desc">Auto-detect and load portable Linux games (.flatimage files) from a folder.</span>
                            </div>
                            <div className="cfg-row__control">
                                <input
                                    type="checkbox"
                                    checked={this.state.flatimage.enabled}
                                    onChange={(e) => this.onFlatImageEnabledChange(e.target.checked)}
                                />
                            </div>
                        </div>
                        {this.state.flatimage.enabled && (
                            <>
                                <div className="cfg-row cfg-row--filepath">
                                    <div className="cfg-row__label">
                                        <span className="cfg-row__name">FlatImage Directory</span>
                                        <span className="cfg-row__desc">Path to the folder containing .flatimage files.</span>
                                    </div>
                                    <div className="cfg-row__control cfg-row__control--wide">
                                        <div className="exodos-path__input">
                                            <input
                                                type="text"
                                                value={this.state.flatimage.flatimageDirectory}
                                                onChange={(e) => this.onFlatImageDirectoryChange(e.target.value)}
                                            />
                                        </div>
                                        <input
                                            type="button"
                                            ...
                                            onClick={this.onFlatImageBrowseClick}
                                        />
                                    </div>
                                </div>
                                <div className="cfg-row">
                                    <div className="cfg-row__label">
                                        ...
                                        <span className="cfg-row__desc">Automatically scan the directory for new .flatimage files on startup.</span>
                                    </div>
                                    <div className="cfg-row__control">
                                        <input
                                            type="checkbox"
                                            checked={this.state.flatimage.autoDetect}
                                            onChange={(e) => this.onFlatImageAutoDetectChange(e.target.checked)}
                                        />
                                    </div>
                                </div>
                            </>
    ...
    onFlatImageEnabledChange = (isChecked: boolean): void => {
        this.setState({ flatimage: { ...this.state.flatimage, enabled: isChecked } });
    };

    onFlatImageDirectoryChange = (value: string): void => {
        this.setState({ flatimage: { ...this.state.flatimage, flatimageDirectory: value } });
    };
    ...
    onFlatImageBrowseClick = (): void => {
        const filePaths = window.External.showOpenDialogSync({
            title: "Select the FlatImage directory",
            properties: ["openDirectory"],
        });
        if (filePaths) {
            this.onFlatImageDirectoryChange(filePaths[0]);
        }
    };
    ...
    onSaveAndRestartClick = (): void => {
        const configData: IAppConfigData = {
            ...
            flatimage: this.state.flatimage,
        };
